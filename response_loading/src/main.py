...
for (q, i) in enumerate(np.arange(0, len(questionCnts), bubblesPerQuestion)):
    # Sort the contours for the current question from
    # left to right, then initialize the index of the
    # bubbled answer
    cnts = contours.sort_contours(questionCnts[i:i + bubblesPerQuestion])[0]
    notBubbled = [None] * bubblesPerQuestion
    bubbled = [None] * bubblesPerQuestion
    ...
    for (j, c) in enumerate(cnts):
        ...
        # bubbled-in answer
        if bubbled[j*(count+1)] is None and total > minimumPixels:
            print total
    ...
    for (i, bubble) in enumerate(bubbled):
        if bubbled[i] is not None:
            ...
            filledInBubbles.update({q:bubbled[i][1]})
...
for i in filledInBubbles:
    print(i, filledInBubbles[i])
